...
def render_result_metrics(results_df):
    ...
    metric_cols[0].metric("Top Confidence", best_confidence["model"], f"{best_confidence['patient_confidence']:.0%}")
    metric_cols[1].metric("Top Val Accuracy", best_validation["model"], f"{best_validation['val_accuracy']:.0%}")
...
def render_input_form():
    ...
    with st.form("patient-form"):
        st.markdown("### ✍️ Enter Patient Features")
...
with right_col:
    ...
    st.markdown('<div class="panel-chip">📈 Model Predictions Overview</div>', unsafe_allow_html=True)
    ...
    # display_df["patient_confidence"] = display_df["patient_confidence"].map("{:.2%}".format)
